...
st.set_page_config(page_title="Ithal Surecler", layout="wide", page_icon="🌍")
...
def check_password():
    ...
    <div style="text-align:center;padding:40px;">
        ...
        <h1 style="background:linear-gradient(135deg,#fa709a,#fee140);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">Ithal Surecler Projesi</h1>
    </div>
...
def get_default_project_data():
    return {
        "FAZ 1: ORNEK FAZ": {
            ...
            "durum": "Planlandı",
            "gorevler": [
                {"id": "I1.1", "gorev": "Ornek gorev 1", "aciklama": "Aciklama", "sure": 2, "baslangic_hafta": 1, "sorumlu": "Sorumlu", "oncelik": "Kritik", "durum": "Planlandı"},
                {"id": "I1.2", "gorev": "Ornek gorev 2", "aciklama": "Aciklama", "sure": 2, "baslangic_hafta": 3, "sorumlu": "Sorumlu", "oncelik": "Yuksek", "durum": "Planlandı"}
            ]
        }
    }
...
if "ithal_verileri" not in st.session_state:
    st.session_state.ithal_verileri = get_default_project_data()

fazlar = st.session_state.ithal_verileri

col1, col2 = st.columns([3, 1])
with col1:
    st.title("🌍 Ithal Surecler Projesi")
    st.caption("2026 CEO Projeleri - Proje #4")
with col2:
    ui = st.session_state.user_info
    st.markdown(f'<div style="text-align:right;padding:10px;background:linear-gradient(135deg,#fa709a,#fee140);border-radius:10px;color:white"><b>{ui["name"]}</b><br><small>{ui["title"]}</small></div>', unsafe_allow_html=True)
...
with tab3:
    ...
    with st.form("yeni_faz"):
        ...
        yf_renk = st.color_picker("Renk", "#fa709a")
        yf_emoji = st.selectbox("Emoji", ["🔴","🟠","🟡","🟢","🔵","🟣"])
        if st.form_submit_button("Faz Ekle"):
            if yf_ad and yf_ad not in fazlar:
                st.session_state.ithal_verileri[yf_ad] = {"baslangic":1,"sure":4,"renk":yf_renk,"emoji":yf_emoji,"durum":"Planlandı","gorevler":[]}
    ...
    with st.form("yeni_gorev"):
        c1, c2 = st.columns(2)
        with c1:
            g_id = st.text_input("ID", placeholder="I1.1")
        ...
        if st.form_submit_button("Gorev Ekle"):
            if g_id and g_gorev:
                st.session_state.ithal_verileri[faz_sec]['gorevler'].append({
...
with tab4:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.download_button("JSON Indir", json.dumps(st.session_state.ithal_verileri, ensure_ascii=False, indent=2), "ithal_surecler.json", "application/json", use_container_width=True)
    with c2:
        up = st.file_uploader("JSON Yukle", type=['json'])
        if up:
            data = json.loads(up.read())
            if st.button("Yukle"):
                st.session_state.ithal_verileri = data
                st.rerun()

st.markdown("---")
st.caption(f"🌍 Ithal Surecler | 2026 CEO Projeleri | {datetime.now().strftime('%d.%m.%Y %H:%M')}")
